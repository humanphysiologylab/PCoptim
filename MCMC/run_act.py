...
#print(np.where(m_index.get_level_values(1) == 'v_rev'))
...
#        s_b_b = sols[len(initial_values_parameters)//13] 
        ...
        new_bounds[np.where(m_index.get_level_values(1) == 'v_half_m')[0][0]] = np.array([15.0, 40.])
        new_bounds[np.where(m_index.get_level_values(1) == 'v_half_h')[0][0]] = np.array([60., 90.])

        new_bounds[np.where(m_index.get_level_values(1) == 'k_m')[0][0]] = np.array([5., 15.])
...
#This is to decrease burn-in. We don't want to start with 'bad' solutions with no Ina. 
    ...
    if check_time < 0.9 and diff_v > 30 and diff_trace_now < diff_no_ina:
...
FOLDER_OUT = '../results/pymc/activation_for_paper'#count_three_starts'
